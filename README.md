...
이 프로젝트는 “운동을 하지 않는 이유” 데이터를 중심으로, 성별·연령·학력·소득·혼인상태·지역별 인구집단이 운동을 하지 않는 주요 원인을 파이차트로 시각화한 분석 프로젝트입니다.
Excel 데이터를 기반으로, 각 그룹의 응답 분포를 한눈에 비교할 수 있도록 파이 그래프로 시각화 환경을 설계했습니다.

---

# 📊 분석 결과(파이 그래프)

아래 이미지는 실제 코드 실행 결과로, 각 인구집단이 운동을 하지 않는 이유의 상대적 비중을 파이차트로 보여줍니다.
...
## 2️⃣ 카테고리(성별/연령/학력/소득/혼인/지역)별 데이터 슬라이싱 자동화

slice_df(df, start, end)함수를 통해 가져온 데이터를 분류에서 파이그래프에 필요한 데이터만 자를 수 있도록 구현했습니다.
...
문항 수에 맞춰 subplot 개수를 자동 계산하여 그래프를 배치하고 어떤 그룹이든 paint_pie(data, header) 한 줄로 전체 문항의 파이그래프 생성 가능하도록 구현했습니다.

## 4️⃣ 한글 폰트와 음수 처리 설정

macOS에서 한글이 깨지는 이슈를 해결하기 위해 AppleGothic를 적용하고, 음수 기호 깨짐 방지를 위해 unicode_minus=False를 적용했습니다.
